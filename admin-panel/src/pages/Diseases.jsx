...
const Diseases = () => {
  ...
  const fetchDiseases = async () => {
    try {
      const response = await axios.get("http://127.0.0.1:8000/api/diseases", { withCredentials: true });
  ...
  const handleAddDisease = async (values) => {
    try {
      await axios.post("http://127.0.0.1:8000/admin_dashboard/add_disease", values, { withCredentials: true });
  ...
  const columns = [
    ...
    { title: "Commonness", dataIndex: "commonness", key: "commonness" },
    {
      ...
      key: "action",
      render: (_, record) => (
        <Button type="primary" danger onClick={() => handleDelete(record.disease_name)}>
          Delete
        </Button>
      ),
    },
